...
def train_config():
    ...
    config = {
        ...
        "reward_weights": {
            ...
            "backlog": 0.01,
            "smooth": 0.5,
            # rail_tat: job 완료 시 그 job이 지나온 rail들에 머문 시간 비중대로 분배되는
            # TAT 초과 페널티(get_step_reward에서 local과 별도 항으로 차감). 신규 항이라
            # 실측 std 기반 보정 전 — 일단 1.0으로 시작, 로그(reward/rail_tat) 보고 재조정.
            "rail_tat": 1.0,
        },
...
class ClientAlgorithm:
    ...
    def get_global_reward(self, pclient):
        ...
        raw = w["tat"] * tat_term + w["op"] * d_op - w["backlog"] * backlog
    ...
    def get_step_reward(self, rail, pclient, g):
        """스텝 보상 = alpha*global + (1-alpha)*local.
        global(g)은 스텝당 1회 계산해 넘겨받고(부작용 1회), local은 라인별로
        local_reward_normalizer로 정규화해서 두 항의 스케일(비율)을 맞춘다."""
        alpha = float(self.config.get("reward_alpha", 0.5))
        l_raw = self.get_local_reward(rail, pclient)
        l = self.local_reward_normalizer.update_and_normalize(l_raw)
        self._last_local_norm = l
        w_rail_tat = float(self.config["reward_weights"].get("rail_tat", 0.0))
        rail_tat_penalty = self._rail_tat_penalty(rail)
        self._last_rail_tat_penalty = rail_tat_penalty
        return alpha * g + (1.0 - alpha) * l - w_rail_tat * rail_tat_penalty
    ...
    def _algorithm_impl(self, pclient):
        ...
        if self.last_batched_states is not None:
            ...
            for i in range(len(sorted_rail_ids)):
                ...
                reward = self.get_step_reward(rail, pclient, g)
                if is_sub_boundary:
