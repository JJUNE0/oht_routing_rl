# contextual-td7-v2 브랜치 실험 기록

## 2026-08-21 - v2.2.0 actor bottleneck environment capture

- `actor_inference`에 `--save_data`/`--save-data` opt-in을 추가했다.
- 평가 시작 후 첫 2,000 tick을
  `results/environment_capture/<capture>/steps.jsonl.gz`에 streaming 저장한다.
  정적 rail topology는 `rail_topology.json.gz`, 진행·완료 상태는
  `manifest.json`에 분리한다.
- 매 tick 모든 rail의 OHT 점유 목록, 예상 유입 OHT, 평균속도, 전송 cost,
  rail별 policy/applied action을 저장한다.
- 모든 OHT의 상태, 현재 rail·거리, simulator 보고 rail 시간, 캡처 중 관측한
  rail 체류시간, stop/idle 시간, job·목적지·route·잔여거리/시간 및 앞차
  정보를 저장한다. active job과 환경/reward/action/runtime 진단도 함께
  저장한다.
- 캡처 overhead는 W&B `runtime/data_capture_ms`로 기록한다.
- Reward N과 network/action/observation/replay/checkpoint 계약은 변경하지
  않았으며 `v2.0.0`/`v2.1.0` artifact는 같은 major 호환 규칙을 유지한다.
- 2-tick synthetic rail/OHT/job capture의 gzip 출력과 dwell 누적을 검증했고
  전체 unittest 258개가 통과했다. 실제 simulator/W&B run은 시작하지 않았다.

## 2026-08-21 - v2.1.0 actor-inference W&B

- `training`과 `actor_inference` 실행은 W&B를 기본 활성화하고,
  `--no-wandb`로 명시적으로 끌 수 있게 변경했다. `baseline_only`는 기본
  비활성화 상태를 유지한다.
- `actor_inference`도 설정된 interval마다 W&B history를 기록한다.
- 평가 profile에서는 env, reward, action, runtime 및 관련 운영 지표를
  유지하고 critic, learner, replay, gradient, SALE 학습 지표는 제외한다.
- actor 평가 runtime에는 state collection, observation build, tensor/device
  transfer, encoder/actor inference, cost apply/send, total latency를 추가했다.
- actor 평가에서 적용되지 않는 exploration noise 표시는 `0.0`으로 고쳤다.
- Reward N, network/action/observation/replay 계약은 변경하지 않았다. SemVer
  규칙에 따라 같은 major의 이전 artifact는 현재 runtime보다 새 버전이
  아니면 호환되며, 기존 fingerprinted `step_400000.pt` 승격도 유지한다.
